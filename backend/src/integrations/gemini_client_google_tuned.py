    ...
    async def _post_image_request(
        ...
        *,
        ...
        try:
            ...
            response = await client.post("/chat/completions", json=_build_payload(image_config=False))

            # Some OpenAI-compatible proxies reject imageConfig. Retry with it only when needed.
            if response.status_code == 400 and "imageconfig" in str(response.text or "").lower():
                logger.warning(
                    "Upstream rejected image request without imageConfig, retrying with imageConfig",
                    extra={"style": style},
                )
                response = await client.post("/chat/completions", json=_build_payload(image_config=True))

            if response.status_code == 400:
                logger.warning(
                    "Image request still fails with 400 after imageConfig fallback",
                    extra={
                        "style": style,
                        "status_code": response.status_code,
                        "response_preview": (response.text or "")[:220],
                    },
                )
...
""" 
    ...
    def _build_fallback_visual_concept(
        self,
        reason: Optional[str],
        message: Optional[str],
    ) -> VisualConcept:
        raw_reason = reason or "professional achievement"
        raw_message = message or "Appreciation for meaningful work"
        theme = raw_reason.split(",")[0][:80] if raw_reason else "professional growth"
        visual_anchor = raw_message if raw_message else raw_reason
        if not visual_anchor:
            visual_anchor = "a collaborative workplace scene"

        return VisualConcept(
            core_theme=theme,
            visual_metaphor=(
                f"{visual_anchor} represented as a clear symbolic object in a professional "
                "setting with stable composition"
            ),
            key_elements=[
                "symbolic workplace object",
                "clear visual hierarchy",
                "light source from side",
                "balanced composition",
                "clean background",
            ],
            mood="appreciative and optimistic",
            composition="centered composition with one dominant motif",
...
"""
    ...
    async def generate_image_direct(
        ...
        visual_concept = self._build_fallback_visual_concept(
            reason=reason,
            message=message,
        )
